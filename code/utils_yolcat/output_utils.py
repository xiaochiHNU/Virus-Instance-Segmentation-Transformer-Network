...
import cv2
from utils_yolcat.box_utils import crop, box_iou, box_iou_numpy, crop_numpy
import torch
import numpy as np
from config import COLORS
from utils_yolcat.cython_nms import nms as cnms
import pdb
import random
...
def draw_img(ids_p, class_p, box_p, mask_p, img_origin, cfg, img_name=None, fps=None):
    ...
    if isinstance(ids_p, torch.Tensor):
        ...
        mask_p = mask_p.cpu().numpy()   # 每个mask都是一个检测框的分割结果（灰度图），如果有10个检测框，那么就有10张mask图

    num_detected = ids_p.shape[0]

    img_fused = img_origin
    if not cfg.hide_mask:
        #n  = (ids_p[:, None, None] + 1)   # 每个检测框的标签信息
        ids_max = (ids_p+1).max()          # 每个检测框的标签最大值
        ids_median = np.median(ids_p + 1)  # 每个检测框的标签中值
        masks_semantic = mask_p * (ids_p[:, None, None] + 1)  # expand ids_p' shape for broadcasting
        #cv2.imshow('mask_p', mask_p[0,:,:])
        #cv2.waitKey(0)
        # The color of the overlap area is different because of the '%' operation.
        masks_semantic = masks_semantic.astype('int').sum(axis=0)
        # 多张masks_semantic相加，数值如果超过ids_max，则赋值ids_median，防止overlap区域大于ids_max
        masks_semantic[masks_semantic > ids_max] = ids_median
        masks_semantic = masks_semantic % (cfg.num_classes)   # (cfg.num_classes - 1)

        color_masks = COLORS[masks_semantic].astype('uint8')
        #cv2.imshow('color_masks', color_masks)
        #cv2.waitKey(0)
        #img_fused = cv2.addWeighted(color_masks, 0.2, img_origin, 0.8, gamma=0)
        img_fused = color_masks   # 直接输出标签图
...
def draw_img1(ids_p, class_p, box_p, mask_p, img_origin, cfg, img_name=None, fps=None):
    if ids_p is None:
        return img_origin
    ...
    if not cfg.hide_mask:
        ...
        ids_max = (ids_p+1).max()          # 每个检测框的标签最大值
        ids_median = np.median(ids_p + 1)  # 每个检测框的标签中值
        random_matrix = np.random.randint(80, size= (num_detected))
        masks_semantic = (mask_p - ids_max + 1) * (random_matrix[:, None, None]+1)   # COLOR (81,3)
        mask_p_max = mask_p.max()
        masks_semantic_max = masks_semantic.max()
        #masks_semantic = mask_p * (ids_p[:, None, None] + 1)  # expand ids_p' shape for broadcasting 随机
        '''
        cv2.imshow('mask_p', mask_p[0,:,:])
        cv2.waitKey(0)

        cv2.imshow('mask_p', mask_p[1, :, :])
        cv2.waitKey(0)

        cv2.imshow('mask_p', mask_p[2, :, :])
        cv2.waitKey(0)

        cv2.imshow('mask_p', mask_p[3, :, :])
        cv2.waitKey(0)

        cv2.imshow('mask_p', mask_p[6, :, :])
        cv2.waitKey(0)

        cv2.imshow('mask_p', mask_p[12, :, :])
        cv2.waitKey(0)
        '''
        # The color of the overlap area is different because of the '%' operation.
        masks_semantic = masks_semantic.astype('int').sum(axis=0)
        masks_semantic_max = masks_semantic.max()
        # 多张masks_semantic相加，数值如果超过ids_max，则赋值ids_median，防止overlap区域大于ids_max,重叠部分取0?
        masks_semantic[masks_semantic > 80] = 0
        masks_semantic_max = masks_semantic.max()
        #masks_semantic = masks_semantic % (cfg.num_classes)   # (cfg.num_classes - 1)
