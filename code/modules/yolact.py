...
from utils_yolcat.box_utils import match, crop, make_anchors
from modules.swin_transformer import SwinTransformer
import pdb
from utils_yolcat.EfficientSwish import MemoryEfficientSwish, Swish
from utils_yolcat.utils_extra import Conv2dStaticSamePadding, MaxPool2dStaticSamePadding
from modules.coordatt import CoordAttplus
...
class DenseFPN(nn.Module):
    def __init__(self, in_channels):
        ...
        self.lat_layers_1 = nn.ModuleList([nn.Conv2d(x, 256, kernel_size=1) for x in self.in_channels])  # lat_layers_1[0]中x对应192, [1]中x对应384, [2]中x对应768
    ...
    def forward(self, backbone_outs):
        p5_1 = self.lat_layers_1[2](backbone_outs[2])
        p5_1_upsample = self.upsample_module[0](p5_1)
        p5_1_upsample2 = self.upsample_module[0](p5_1_upsample)
        p5_3 = self.lat_layers_3[2](backbone_outs[2])

        p4_1 = self.lat_layers_1[1](backbone_outs[1])
        p4_1_upsample = self.upsample_module[0](p4_1)
        p4_3 = self.lat_layers_3[1](backbone_outs[1]) + p5_1_upsample
        ...
        p6_out = self.downsample_layers[0](p5_out)
        p7_out = self.downsample_layers[1](p6_out)
