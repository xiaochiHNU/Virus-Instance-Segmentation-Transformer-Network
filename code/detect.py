...
from utils_yolcat.coco import COCODetection, detect_collate
from utils_yolcat import timer
from utils_yolcat.output_utils import nms, after_nms, draw_img
from utils_yolcat.common_utils import ProgressBar
from utils_yolcat.augmentations import val_aug

parser = argparse.ArgumentParser(description='YOLACT Detection.')
parser.add_argument('--weight', default='weights/loss_0.47_swin_transformer_7850.pth', type=str)
parser.add_argument('--image', default='data/virus_coco/test', type=str, help='The folder of images for detecting.')  # None
...
parser.add_argument('--no_crop', default=True, action='store_true',
...
parser.add_argument('--visual_thre', default=0.7, type=float,
                    help='Detections with a score under this threshold will be removed.')

if __name__ == '__main__':
    args = parser.parse_args()
    prefix = re.findall(r'loss_\d+\.\d+_', args.weight)[0]
    ...
    if cfg.image is not None:
        ...
        data_loader = data.DataLoader(dataset, 1, num_workers=0, shuffle=False, pin_memory=True, collate_fn=detect_collate)
        ds = len(data_loader)
